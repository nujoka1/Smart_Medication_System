...
def _run_pill_ai_observation(image_path):
    ...
    try:
        ...
        try:
            ...
            try:
                detector = PillDetector(conf=0.15)
            except TypeError:
...
def _draw_annotated_evidence(raw_path, annotated_path, metadata):
    ...
    # Draw all model detections if available.
    # If there are no detections but a single bbox exists, draw that one.
    detections = model.get("detections") or []

    if not detections and model.get("bbox"):
        detections = [{
            "class": detected,
    ...
    for det in detections[:8]:
        ...
        try:
            ...
            det_class = det.get("class") or detected or "pill"
            det_conf = det.get("confidence")

            try:
                det_conf_text = f"{float(det_conf) * 100:.1f}%"
            except Exception:
                det_conf_text = "N/A"

            draw.rectangle([x1, y1, x2, y2], outline=(0, 255, 120), width=4)

            label = f"{det_class} {det_conf_text}"
            label_w = min(w - int(x1), 390)
            label_top = max(0, int(y1) - 32)

            draw.rectangle(
                [int(x1), label_top, int(x1) + label_w, label_top + 30],
                fill=(8, 8, 22)
            )
            draw.text(
                (int(x1) + 6, label_top + 5),
                label,
                fill=(0, 255, 120),
                font=font_small
            )
...
def evidence_capture():
    ...
    dose_period = d.get("dose_period")
    dose_quantity = d.get("dose_quantity")
    compartment = d.get("compartment")
    ir_target_count = d.get("ir_target_count", 0)
    ir_actual_count = d.get("ir_actual_count", 0)

    if not dose_period and dose_time:
        try:
            hour = int(str(dose_time).split(":")[0])
            if 5 <= hour < 12:
                dose_period = "Morning dose"
            elif 12 <= hour < 17:
                dose_period = "Afternoon dose"
            elif 17 <= hour < 22:
                dose_period = "Evening dose"
            else:
                dose_period = "Night dose"
        except Exception:
            dose_period = "Scheduled dose"
